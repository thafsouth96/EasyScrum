$(document).ready(function(){
 ...
 //Toggle the description of a release à modifier
  ...
  $('#content').on('click', '.edit-release', function(){
    var releaseToEdit_id = $(this).parents('.releaseName').children('.hidden_release_id').text();
    var current_release_name = $(this).parents('.releaseName').children('.rName').text();
    var current_release_desc = $(this).parents('.releaseColonne').children('.toggleDivDesc').children('.releaseRecap').text();

    $('#edit_release_nom').val(current_release_name);
    $('#edit_release_description').val(current_release_desc);

    if(!$('.hidden_input_release_id').length){
        $('#formEditReleaseContent').append('<input type="text" class="hidden_input_release_id" name="releaseToEdit_id" value="'+ releaseToEdit_id +'">');
    } else {
       $('.hidden_input_release_id').val(releaseToEdit_id);
    }
  });
});
